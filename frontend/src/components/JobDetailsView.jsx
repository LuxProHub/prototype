  ...
  return (
    <div className="p-4 sm:p-6 h-full w-full max-w-[1520px] mx-auto flex flex-col min-h-0 overflow-hidden space-y-4 animate-fade-in">
      ...
      <div className="flex-1 min-h-0 grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-5 overflow-hidden">
        ...
        <div className="lg:col-span-1 bento-card p-4 rounded-[var(--r-lg)] flex flex-col min-h-0 overflow-hidden">
          <div className="flex items-center justify-between border-b border-[var(--edge)] pb-3 shrink-0">
              ...
              return (
                ...
                >
                  ...
                  <div className="flex justify-between t-meta num mt-1">
                    ...
                    <span className={job.error_rows > 0 ? 'text-[var(--bad)] font-semibold' : ''}>
        ...
        <div className="lg:col-span-2 flex flex-col min-h-0 overflow-hidden space-y-4">
          {activeJobData ? (
            <>
              ...
              <div className="bento-card p-4 sm:p-5 rounded-[var(--r-lg)] space-y-3 shrink-0">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-[var(--edge)] pb-3">
                  <div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 divide-x divide-[var(--edge)] text-xs">
                  <div className="px-3 py-1.5 first:pl-0">
                    <span className="text-[var(--text-3)] t-label block">Total rows</span>
                    <span className="text-[var(--text)] font-bold text-sm num">
                      {activeJobData.total_rows?.toLocaleString()}
                    </span>
                  </div>
                  <div className="px-3 py-1.5 first:pl-0">
                    <span className="text-[var(--ok)] t-label block">Valid</span>
                    <span className="text-[var(--ok)] font-bold text-sm num">
                      {activeJobData.valid_rows?.toLocaleString()}
                    </span>
                  </div>
                  <div className="px-3 py-1.5 first:pl-0">
                    <span className="text-[var(--warn)] t-label block">Duplicates</span>
                    <span className="text-[var(--warn)] font-bold text-sm num">
                      {activeJobData.duplicate_rows?.toLocaleString()}
                    </span>
                  </div>
                  <div className="px-3 py-1.5 first:pl-0">
                    <span className="text-[var(--bad)] t-label block">Errors</span>
                    <span className="text-[var(--bad)] font-bold text-sm num">
              ...
              <div className="bento-card p-4 sm:p-5 rounded-[var(--r-lg)] flex-1 flex flex-col min-h-0 overflow-hidden space-y-3">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-[var(--edge)] pb-3 shrink-0">
                  <div className="flex items-center gap-2">
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="field h-7 pl-8 pr-2 text-xs w-48 rounded-[var(--r-sm)]"
                    />
                  </div>
                </div>

                <div className="overflow-auto flex-1 min-h-0 rounded-[var(--r-md)] border border-[var(--edge)]">
                      ...
                      {filteredErrors.length > 0 ? (
                        filteredErrors.map((errItem, idx) => (
                          <tr key={idx} className="hover:bg-[var(--surface-2)]">
                            <td className="p-2.5 font-mono text-[var(--text-2)] font-semibold num">
                              {errItem.batch || 1}
                            </td>
                            <td className="p-2.5 font-mono text-[var(--text-2)] num">
                              {errItem.row_index || errItem.row}
                            </td>
                            <td className="p-2.5 font-mono text-[var(--bad)] font-semibold">
                              {errItem.field || 'General'}
                            </td>
                            <td className="p-2.5 text-[var(--text-2)] font-medium">
                        ...
                        <tr>
                          <td
                            colSpan="4"
                            className="py-10 text-center text-[var(--text-3)] font-mono text-xs"
                          >
                            {errors.length === 0
            ...
            <div className="bento-card p-8 text-center text-[var(--text-3)] font-mono text-xs flex-1 flex items-center justify-center">
