...
function Tile({ label, value, sub, Icon, tone = 'text-[var(--text)]' }) {
  return (
    <div className="panel rounded-lg p-4 flex-1 min-w-[150px]">
      <div className="flex items-center justify-between">
        <span className="text-[10px] font-mono text-[var(--text-3)] font-semibold">{label}</span>
        {Icon && <Icon className="w-3.5 h-3.5 text-[var(--text-3)]" aria-hidden="true" />}
      </div>
    ...
    <div className="flex-1 overflow-y-auto p-6 space-y-5">
          ...
          {[7, 30, 90].map((d) => (
            <button key={d} onClick={() => setDays(d)} aria-pressed={days === d}
                    className={`px-3 py-1.5 rounded-lg text-xs font-semibold ${
                      days === d ? 'btn-primary' : 'btn text-[var(--text-2)]'}`}>
              {d}d
            </button>
          ))}
        </div>
      ...
      {pipeline && (
        <div className="flex flex-wrap gap-3">
          ...
          <Tile label="OVERDUE ACTIONS" value={pipeline.overdue_actions}
                Icon={AlertTriangle}
                tone={pipeline.overdue_actions ? 'text-[var(--bad)]' : 'text-[var(--text)]'}
          ...
          <Tile label="CONTACTS DISPROVED" value={pipeline.contacts_disproved}
                Icon={ShieldCheck} tone="text-[var(--ok)]"
      ...
      {team && (
        <div className="space-y-2">
          <h3 className="text-sm font-semibold text-[var(--text)]">By person</h3>
          {!team.people.length && (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-xs">
              <thead>
                <tr className="text-[10px] font-mono text-[var(--text-3)] text-left">
              ...
              <tbody>
                {team.people.map((p) => (
                  <tr key={p.user} className="border-t border-[var(--edge)]">
                    <td className="py-2 pr-3 font-semibold text-[var(--text)]">{p.user}</td>
                    ...
                    <td className="py-2 pr-3 text-[var(--ok)] font-semibold">
      ...
      {pipeline && (
        <div className="space-y-2">
          ...
          <div className="flex flex-wrap gap-2">
            {STAGE_ORDER.map((s) => (
              <div key={s} className="panel rounded-xl px-3 py-2">
                <div className="text-[10px] font-mono text-[var(--text-3)] font-semibold">
                  {s.replace(/_/g, ' ')}
                </div>
